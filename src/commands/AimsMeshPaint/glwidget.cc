...
myGLWidget<T>::myGLWidget(QWidget *parent, string adressTexIn,
		string adressMeshIn, string adressTexOut, string colorMap,
		string dataType) :
	GLWidget(parent), _adressTexIn(adressTexIn), _adressMeshIn(adressMeshIn),
			_adressTexOut(adressTexOut), _colorMap(colorMap), _dataType(
					dataType) {
	_zoom = -2.0;
	_trans = 0.0;
	_mode = 1;
	...
	_trackBall = TrackBall(0.05f, gfx::Vector3f::vector(0, 1, 0),
			TrackBall::Sphere);
	_trackBall = TrackBall(0.0f, gfx::Vector3f::vector(0, 1, 0),
			TrackBall::Plane);
...
int myGLWidget<T>::buildDisplayList(AimsSurfaceTriangle as, int mode) {
	...
	for (j = 0; j < (int) tri.size(); ++j) {
		...
		if (mode == 1)
			glColor3ub(_indexTexture[3 * j], _indexTexture[3 * j + 1],
					_indexTexture[3 * j + 2]);
...
void myGLWidget<T>::buildDataArray(void) {
	...
	for (j = 0; j < (int) vert.size(); j++) {
		_vertices[3 * j]
				= (GLfloat)(_meshScale * (vert[j][X] - _meshCenter[X]));
		_vertices[3 * j + 1] = (GLfloat)(_meshScale * (vert[j][Y]
				- _meshCenter[Y]));
		_vertices[3 * j + 2] = (GLfloat)(_meshScale * (vert[j][Z]
				- _meshCenter[Z]));
...
void myGLWidget<T>::mouseMoveEvent(QMouseEvent *event) {
	...
	if (_mode == 2) {
		...
		_indexVertex = computeNearestVertexFromPolygonPoint(p, _indexPolygon,
				_mesh);
