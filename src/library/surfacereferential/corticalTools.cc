...
// 	Writer<Texture1d> w("test_pole_sud.tex");
// 	w.write(distance_sud);
// 	Writer<Texture1d> w1("test_pole_nord.tex");
			...
			if( it_lat2!=map_global["lat"].end() )
			{
				...
				for(; it_lon!=map_global["lon"].end(); ++it_lon)
				{
					...
					if( it_lon2!=map_global["lon"].end() )
						std::cout<<(*it_lon)<<" - "<<(*it_lon2)<<std::endl;
				}
			}
		}*/
...
// 		std::cout<<"compteur="<<cpt/size<<std::endl;
