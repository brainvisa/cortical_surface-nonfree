...
vector<vector<uint> > sortVertices(uint gyruslabel, const vector<set<uint> > &voisins, const Texture<short> &inTex) {
   ...
   // m�thode qui pour un gyrus donn� renvoit les indices des points du gyrus, des points du bord du gyrus, des points du bord �tant voisins de deux gyri distincts
...
vector<uint> getIntersection(short gyruslabel, short voisin1, short voisin2, const vector<set<uint> > &voisins,
   ...
   // cette m�thode renvoit les points d'une couleur donn�e ayant pour voisins des points de deux couleurs donn�es
...
vector<uint> getIntersection(short gyruslabel, short voisin1, const vector<set<uint> > &voisins, const Texture<short> &inTex){
    // cette m�thode renvoit les points d'une couleur donn�e ayant pour voisins des points d'une couleur donn�e
...
vector<uint> getThirdPoints(uint a, uint b, const vector<set<uint> > &voisins){
   ...
   // m�thode qui pour deux indices de points donn�s renvoit la liste de points voisins de l'un et de l'autre simultan�ment
...
vector<uint> getBorderNeighbours(uint v, const vector<uint> &borderVertices, const vector<set<uint> > &voisins){
   ...
   // � un point en entr�e la m�thode renvoit ses voisins contenus dans le bord
...
vector<uint> isolineExtraction(double value, AimsSurfaceTriangle &gyrusMesh, const Texture<double> &inTex){
   ...
   // m�thode qui renvoit une liste d'indice de points correspondant 
...
uint getNearestPoint(const vector<uint> &vec, const Texture<double> &inTex, double value){
   ...
   // � partir d'une liste d'indices de points, d'une texture et d'une valeur donn�e, renvoit le point dont la valeur sur la texture est la plus proche de la valeur cible
