...
bool verifGyriExistence(short g, const Point3d &gHaut, const Point3d &gBas, const vector<uint> &corres){
   uint test=0;
   if (find(corres, g)==-1){
      cerr << "Attention : le gyrus " << g << " r�f�renc� dans le mod�le de diffusion n'existe pas dans la parcellisation.\n" << endl;
      test=1;
   }
   for (uint i=0;i<3;i++)
      if (find(corres, gHaut[i])==-1) {
         cerr << "Attention : le gyrus " << gHaut[i] << " r�f�renc� dans le mod�le de diffusion n'existe pas dans la parcellisation.\n" << endl;
         test=1;
      }
      else if (find(corres, gBas[i])==-1){
         cerr << "Attention : le gyrus " << gBas[i] << " r�f�renc� dans le mod�le de diffusion n'existe pas dans la parcellisation.\n" << endl;
         test = 1;
      }
   return (test==0);
}
