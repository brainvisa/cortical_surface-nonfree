...
//    _texBassinsDepthNorm[0].item(i) = 1;
  ...
  if (_adrBassinsDepthNorm=="")
  {
    ...
    string toto1 = "texBassinsDepthNorm_e10.tex";
    Writer<TimeTexture<float> > texW1(toto1);
    texW1 << _texBassinsDepthNorm;
  }
  ...
  //on dilate les roots
   ...
   // on fait les intersections avec les bassins
    ...
    TimeTexture<float> _texProbaPathNormLatThresh(1, _mesh.vertex().size() );
        ...
        GeodesicPath sp(_mesh,_texBassinsDepthNorm,1,-3);
        ...
        for (; mit2 != mend2; ++mit2)
        {
          ...
          if ((mit2->second).size() > 150)
          {
            ...
            {
              //listVertexBassin2++;
              if (_texProbaPathNormLatThresh[0].item(*listVertexBassin2)==1)
                listIndexVertexTarget.push_back(*listVertexBassin2);
            }
...
//
    ...
    for (; mit2 != mend2; ++mit2)
    {
      ...
      if ((mit2->second).size() > 150)
      {
        ...
        {
          //listVertexBassin2++;
          if (_texProbaPathNormLonThresh[0].item(*listVertexBassin2)==1)
            listIndexVertexTarget.push_back(*listVertexBassin2);
        }
